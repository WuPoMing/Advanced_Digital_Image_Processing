...
Mat bicubic(Mat img, double rx, double ry) {
    ...
    // output image
    Mat out = Mat::zeros(resized_height, resized_width, CV_8UC1);
            ...
            //for (int c = 0; c < channel; c++) {
                ...
                for (int j = -1; j < 3; j++) {
                    ...
                    for (int i = -1; i < 3; i++) {
                        ...
                        wx = h(fabs(dx - _x));
                        w_sum += wy * wx;
                        //val += (double)img.at<Vec3b>(_y, _x)[c] * wx * wy;
                        val += (double)img.at<uchar>(_y, _x) * wx * wy;
                    }
                }
